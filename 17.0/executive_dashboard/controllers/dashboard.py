...
class ExecutiveDashboardController(http.Controller):
    ...
    def _get_sales_kpi(self, df, dt, prev_df, prev_dt, company_id):
        ...
        return self._build_kpi(current['total'], current['count'], previous['total'],
                               'Ventas', request.env.company.currency_id.symbol)
    ...
    def _get_invoicing_kpi(self, df, dt, prev_df, prev_dt, company_id):
        ...
        result = self._build_kpi(current['total'], current['count'], previous['total'],
                                 'Facturación', request.env.company.currency_id.symbol)
    ...
    def _get_purchases_kpi(self, df, dt, prev_df, prev_dt, company_id):
        ...
        previous = cr.dictfetchone()
        return self._build_kpi(current['total'], current['count'], previous['total'],
                               'Compras', request.env.company.currency_id.symbol)
    ...
    def _get_stock_kpi(self, company_id):
        ...
        return {
            ...
            'formatted': self._format_currency(value, request.env.company.currency_id.symbol),
            'trend_pct': None,
            'label': 'Valor Stock',
        }
    ...
    def _get_margin_kpi(self, df, dt, prev_df, prev_dt, company_id):
        ...
        env.cr.execute("""
            ...
            WHERE table_name = 'sale_order_line' AND column_name IN ('margin', 'purchase_price')
            LIMIT 1
        """)
        has_margin_col = env.cr.fetchone() is not None

        if has_margin_col and self._column_exists('sale_order_line', 'margin'):
            env.cr.execute("""
        ...
        domain = [('order_id.state', 'in', ['sale', 'done']),
                  ...
                  ('order_id.date_order', '>=', df), ('order_id.date_order', '<=', dt)]
        lines = env['sale.order.line'].search(domain)
        revenue = sum(l.price_subtotal for l in lines)
        domain_prev = [('order_id.state', 'in', ['sale', 'done']),
                       ('order_id.company_id', '=', company_id),
                       ('order_id.date_order', '>=', prev_df), ('order_id.date_order', '<=', prev_dt)]
        lines_prev = env['sale.order.line'].search(domain_prev)
        prev_revenue = sum(l.price_subtotal for l in lines_prev)
        kpi = self._build_kpi(revenue, len(lines), prev_revenue, 'Ventas Netas', currency)
    ...
    def _get_sales_evolution(self, df, dt, company_id):
        """
        Monthly sales vs invoicing chart.
        Si no hay ventas, muestra solo facturación — el gráfico nunca queda vacío.
        """
        cr = request.env.cr
        ...
        """, (chart_from, chart_to, company_id, chart_from, chart_to,
              company_id, chart_from, chart_to))
        ...
        # Si no hay ventas en absoluto, devolver solo facturación como serie única
        if sum(sales) == 0:
            return {
                'labels': labels,
                'values': invoicing,
    ...
    def _get_sales_by_category(self, df, dt, company_id):
        """
        Ventas por categoría. Si no hay ventas, muestra facturación por cliente (top 6).
        """
        cr = request.env.cr
        cr.execute("""
        ...
        # Si no hay ventas, usar facturación por cliente como fallback
        if not rows or sum(float(r['total']) for r in rows) == 0:
            cr.execute("""
    ...
    def _get_alerts(self, company_id):
        ...
        # 2. Productos bajo punto de reorden
        cr.execute("""
        ...
        # 3. Órdenes de compra con entrega pendiente
        cr.execute("""
    ...
    def _payment_status_label(self, state):
        ...
        return mapping.get(state, {'label': state, 'color': 'gray'})
